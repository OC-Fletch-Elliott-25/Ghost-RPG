...
func enemy_broadsides():
	...
	$Enemy_Attack_Label.text = "The Flying Dutchman fires its broadsides!"
	await get_tree().create_timer(1).timeout
	while ammo > 0:
		await get_tree().create_timer(1).timeout
		#if the player isn't dead, the enemy then rolls damage which can be anywhere from 5 to 50 damage.
